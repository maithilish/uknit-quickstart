...
public class Quickstart {
	...
	public StringBuilder chainedMethodCall(final StringBuilder s1, final StringBuilder s2, final File file) {
	...
	public void conditional(final Foo foo, final File file) {
		String x = "x";
		String y = "y";
		foo.appendString(file.getAbsolutePath().equals(x) ? foo.format(x) : foo.format(y));
		foo.appendString(file.getAbsolutePath().equals(y) ? foo.format(x) : foo.format(y));
	}
